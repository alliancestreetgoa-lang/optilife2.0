...
// Real 3D product bottles (WebGL) — client-only.
const HeroBottles = dynamic(() => import('@/components/three/HeroBottles'), { ssr: false })

export function Hero() {
  return (
    <section className="relative min-h-[100svh] flex items-start lg:items-center overflow-hidden">
      {/* 3D product bottles — lower band on mobile, right side on desktop */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        className="absolute z-[5] pointer-events-none
          ...
          lg:inset-x-auto lg:bottom-auto lg:right-0 lg:top-1/2 lg:h-[88%] lg:w-[52%] lg:-translate-y-1/2"
        aria-hidden="true"
      >
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(58,125,60,0.14),transparent_62%)]" />
        <HeroBottles />
      </motion.div>
